...
open class Chart: UIControl {
    ...
    */
    ...
    fileprivate func drawChart() {
        ...
        for (index, series) in self.series.enumerated() {
            ...
            segments.forEach({ segment in
                ...
                if series.showDots {
                    drawDots(scaledXValues, yValues: scaledYValues, seriesIndex: index)
                }
            })
        }
    ...
    fileprivate func getMinMax() -> (min: ChartPoint, max: ChartPoint) {
        ...
        // If not provided, find them in the series
        for s in series {
            let xValues = s.data.map { $0.x }
            let yValues = s.data.map { $0.y }

            if min.x == nil || xValues.minOrZero() < min.x! { min.x = xValues.minOrZero() }
            if max.x == nil || xValues.maxOrZero() > max.x! { max.x = xValues.maxOrZero() }
            if min.y == nil || yValues.minOrZero() < min.y! { min.y = yValues.minOrZero() }
            if max.y == nil || yValues.maxOrZero() > max.y! { max.y = yValues.maxOrZero() }
        }

        // Default values if no series
    ...
    fileprivate func scaleValuesOnXAxis(_ values: [Double]) -> [Double] {
        ...
        let factor = width / (max.x - min.x)
        return values.map { ($0 - min.x) * factor }
    }

    fileprivate func scaleValuesOnYAxis(_ values: [Double]) -> [Double] {
        let height = Double(drawingHeight)
        let factor = height / (max.y - min.y)
        return values.map { Double(drawingHeight) - ($0 - min.y) * factor + Double(topInset) }
    }

    fileprivate func getZeroValueOnYAxis(zeroLevel: Double) -> Double {
        let height = Double(drawingHeight)
        let factor = height / (max.y - min.y)
        return Double(drawingHeight) - (zeroLevel - min.y) * factor + Double(topInset)
    }
    ...
    fileprivate func drawLine(_ xValues: [Double], yValues: [Double], seriesIndex: Int) {
        ...
        let isAboveZeroLine = yValues.first! <= getZeroValueOnYAxis(zeroLevel: series.colors.zeroLevel)
        
        lineLayer.strokeColor = isAboveZeroLine ? series.colors.above.cgColor : series.colors.below.cgColor
        lineLayer.fillColor = nil
        lineLayer.lineWidth = lineWidth
        
        if series.dashed {
            lineLayer.lineDashPattern = series.dashPattern
        }
    ...
    fileprivate func drawDots(_ xValues: [Double], yValues: [Double], seriesIndex: Int) {
        let series = self.series[seriesIndex]
        let radius = series.dotRadius
        
        for i in 0..<xValues.count {
            let dotPath = UIBezierPath(arcCenter: CGPoint(x: CGFloat(xValues[i]), y: CGFloat(yValues[i])), radius: radius, startAngle: 0, endAngle: CGFloat.pi * 2, clockwise: true)
            let dotLayer = CAShapeLayer()
            dotLayer.path = dotPath.cgPath
            
            let isAboveZeroLine = yValues[i] <= getZeroValueOnYAxis(zeroLevel: series.colors.zeroLevel)
            dotLayer.fillColor = isAboveZeroLine ? series.colors.above.cgColor : series.colors.below.cgColor
            
            self.layer.addSublayer(dotLayer)
            layerStore.append(dotLayer)
        }
    }

    fileprivate func drawArea(_ xValues: [Double], yValues: [Double], seriesIndex: Int) {
        let series = self.series[seriesIndex]
        let zero = CGFloat(getZeroValueOnYAxis(zeroLevel: series.colors.zeroLevel))
        let isAboveZeroLine = yValues.first! <= zero

        let area = CGMutablePath()
        area.move(to: CGPoint(x: CGFloat(xValues.first!), y: zero))
        for i in 0..<xValues.count {
        ...
        if isAboveZeroLine {
            areaLayer.fillColor = series.colors.above.withAlphaComponent(areaAlphaComponent).cgColor
        } else {
            areaLayer.fillColor = series.colors.below.withAlphaComponent(areaAlphaComponent).cgColor
        }
    ...
    */
    fileprivate class func intersectionWithLevel(_ p1: ChartPoint, and p2: ChartPoint, level: Double) -> ChartPoint {
        let dy1 = level - p1.y
        let dy2 = level - p2.y
        if dy1 == dy2 { return (x: p1.x, y: level) }
